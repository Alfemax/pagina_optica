...
/* ------------------- Helpers & Plantillas ------------------- */
const FROM = process.env.SMTP_FROM || `"Clínica El Áncora" <${process.env.SMTP_USER}>`;
...
// Base de email visual (responsive simple + estilos inline seguros)
function baseTemplate({ title, intro, content, cta, footerNote }) {
  return `
  <div style="margin:0;padding:0;background:#0ea5e90d">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;">
      <tr>
        <td>
          <!-- Header -->
          <table role="presentation" align="center" width="100%" style="max-width:640px;margin:0 auto;">
            <tr>
              <td style="padding:24px 20px 0 20px;text-align:center;">
                <img src="https://i.imgur.com/R6n9JZp.png" alt="Clínica El Áncora" width="52" height="52" style="border-radius:12px;display:inline-block;box-shadow:0 8px 18px rgba(14,165,233,.25)">
                <h1 style="margin:12px 0 0 0;font-size:22px;line-height:1.2;color:#0f172a">Clínica El Áncora</h1>
              </td>
            </tr>
          </table>

          <!-- Card -->
          <table role="presentation" align="center" width="100%" style="max-width:640px;margin:16px auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden;box-shadow:0 16px 36px rgba(2,6,23,.08)">
            <tr>
              <td style="background:linear-gradient(135deg,#0ea5e9,#22d3ee);height:6px"></td>
            </tr>
            <tr>
              ...
              <td style="padding:8px 22px 18px 22px;color:#1f2937;font-size:15px;line-height:1.55">
                ...
                ${cta ? `
                  <div style="margin-top:16px">
                    <a href="${cta.href}" style="display:inline-block;background:#0ea5e9;color:#fff;text-decoration:none;padding:10px 16px;border-radius:999px;font-weight:700;box-shadow:0 8px 18px rgba(14,165,233,.35)">${cta.label}</a>
                  </div>` : ''}
              </td>
            </tr>
          </table>
          ...
          <!-- Footer -->
          <table role="presentation" align="center" width="100%" style="max-width:640px;margin:12px auto 24px">
            <tr>
              <td style="padding:0 20px;text-align:center;color:#64748b;font-size:12px;line-height:1.4">
                ${footerNote || '11 Calle 5-75, Zona 1 · Tel. +502 2232-2721 · WhatsApp +502 4144-5224'}
              </td>
            </tr>
          </table>
        </td>
...
export async function sendVerificationEmail({ to, code }) {
  const html = baseTemplate({
    ...
    content: `
      <div style="margin:10px 0 0 0;padding:12px 14px;border:1px dashed #94a3b8;border-radius:12px;text-align:center">
...
export async function sendResetEmail({ to, code }) {
  const html = baseTemplate({
    ...
    content: `
      ...
      <div style="margin:8px 0 0 0;padding:12px 14px;border:1px dashed #94a3b8;border-radius:12px;text-align:center">
        <div style="letter-spacing:6px;font-weight:800;font-size:22px;color:#0ea5e9">${code}</div>
...
export async function sendRecipeEmail({ to, nombre, id_receta, pdfBuffer }) {
  ...
  return sendMail({
    ...
    html,
    attachments: [
      { filename: `receta_${id_receta}.pdf`, content: pdfBuffer }
    ]
  });
}
